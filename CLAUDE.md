...
- **The author's machine has PHP 8.3 but no `php.ini`**, so no extensions are enabled by default. Composer and Symfony need at least `openssl`, `mbstring`, `curl`, `zip`, `intl`, `fileinfo`, `pdo_pgsql`.
- Docker is not installed on the author's machine yet — `docker compose up` has never been run and is unverified. Same for the CI workflow: it has never executed.
